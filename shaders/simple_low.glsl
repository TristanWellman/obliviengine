...
void main() {
	...
	viewSpaceNorm = mat3(view) * normal;
	texcoord = texcoord0;
}
...
void main() {
    ...
    lowp vec3 viewDir = camPos - fragPos;
	...
	int i;
    for(i=0;i<al;i++) {
		lowp float d = dot(vec3(positions[i])-fragPos,
				vec3(positions[i])-fragPos);
		lowp float atten = 1.0/(1.0+d);

        lowp vec3 lightDir = vec3(positions[i]) - fragPos;
        lowp float diff = max(dot(norm, lightDir), 0.0);
        lowp vec3 diffuse = 
			vec3(colors[i]*colors[i].a)*MATDIFF*diff*atten;
		ad += diffuse;
    }

    frag_color = vec4((AMB+ad*texcolor)*vec3(color), 1.0);
